...
class Composer:
    """Call compose to start composing a new pattern"""
    w = r"a-zA-Zα-ωΑ-Ω0-9"
    ...
    def ogroup(self) -> str:
        """
        Optional LaTeX argument `[n]`\n
        `group \\2` ... argument group\n
        \+2
        """
        self.n += 2
        return fr"<LO(\d+)>(.*)<RO\{self.n-1}>"
    ...
    def differential(self) -> str:
        """
        group \\2 ... integration variable
        \+2
        """
        self.n += 2
        return fr"(?:d|<TE><LL(\d+)>d<RR\{self.n-1}>)([a-zA-Zα-ωΑ-Ω])" # + self.opt_base() + self.opt_exponent()
