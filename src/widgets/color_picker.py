...
from utils.color_parser import str_to_qcolor
...
class ColorPort(QWidget):
    ...
    def selectColor(self):
        ...
        # Hue and Saturation are derived from the position
    ...
    def setColor(self, color: QColor):
        ...
        self.setHue(self.hue) if s == 0 else self.setHue(color.hue())
        return
...
class HueSlider(QSlider):
    def __init__(self, orientation: Qt.Orientation, parent=None):
        ...
        self.valueChanged.connect(self.setHue)
        return
...
class ColorPicker(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.hue_slider.valueChanged.connect(self._hueChanged)
        self.color_port.colorChanged.connect(self._colorChanged)
    ...
    def setColor(self, color: QColor | str) -> None:
        self.color_port.setColor(color)
        # Prevent gradient from becoming grayscale
        if color.saturation() == 0:
            self.hue_slider.setHue(self.color_port.hue)
        else:
            self.hue_slider.setHue(color.hue())
        return

    def _hueChanged(self, hue) -> None:
        self.color_port.setHue(hue)
        self.color_display.setColor(self.color_port.color)
        return
    
    def _colorChanged(self, color) -> None:
